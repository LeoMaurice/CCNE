---
title: "Résumé des citations des avis du CCNE"
...
main_plot <- ggraph(layout) +
  ...
  # theme(
  # plot.caption =  element_text(hjust = 1, vjust = -2, margin = margin(t = 20, r = 20, b = 20, l = 0))
  # )

# Plot vide avec la légende seule
caption_plot <- ggplot() +
  labs(caption = "Graphique de l'auteur.\nChamps : Tous les avis numérotés publiés par le CCNE jusqu'au 144 et leurs citations.\nDonnées : citations collectées par l'auteur.\nLecture: Les lois de bioéthique, dans la catégorie Loi, sont citées 26 fois dans les avis.") +
  theme_void() +
  theme(
...
final_plot <- main_plot / caption_plot + plot_layout(heights = c(1, 0.1))
final_plot
```
...
plot_data_nb_citations%>%
  ...
  ggscatter(x = "date", y = "nb_citation", facet.by = "Catégorie", color = "president",size = 2.5, xlab = "Temps",ylab = "Nombre de citations") +
  scale_color_prism("black_and_white")+
  theme_ready(text_size = 27)
```
# BW
...
# metadata_avis <- metadata_avis %>%
#   mutate(president = case_when(
#     num >= 1 & num <= 28 ~ "Bernard (1983-1991)",
#     num >= 29 & num <= 60 ~ "Changeux (1992-1999)",
#     num >= 61 & num <= 103 ~ "Sicard (2000-2008)",
#     num >= 104 & num <= 115 ~ "Grimfeld (2009-2011)",
#     num >= 116 & num <= 124 ~ "Ameisen (2012-2015)",
#     num >= 125 & num <= 144 ~ "Delfraissy (2016-)"
#   ))
# 
# # Assurez-vous que les présidents sont dans le bon ordre
# metadata_avis$president <- factor(metadata_avis$president, levels = c(
#   "Bernard (1983-1991)", "Changeux (1992-1999)", "Sicard (2000-2008)", "Grimfeld (2009-2011)", "Ameisen (2012-2015)", "Delfraissy (2016-)"
# ))
# 
# all_combinations <- expand.grid(
#   num = 1:144,
...
#   left_join(metadata_avis, by = "num") -> plot_data_nb_citations
# 
# plot1 <- plot_data_nb_citations%>%
#   filter(Catégorie %in% c("Autorités", "Etat", "Comparaison pays", "Société", "CCNE")) %>%
#   mutate(Catégorie = ifelse(Catégorie == "Comparaison pays", "Comparaison\npays", Catégorie)) %>%
#   ggboxplot(x = "Catégorie", y = "nb_citation", fill = "president", xlab = "", ylab = "") +
#   labs(fill = "Président") +
#   # ggscatter(x = "date", y = "nb_citation", facet.by = "Catégorie", color = "president",size = 2.5, xlab = "Temps",ylab = "Nombre de citations") +
#   scale_fill_manual(values = c("grey10", "gray50", "gray30", "gray80", "gray70", "gray100")) +
#   theme_ready(text_size = 28)
```
```{r}
# plot2 <- plot_data_nb_citations%>%
#   filter(Catégorie %in% c("Comité d'éthique", "Forums", "Loi", "Org Internationales", "Presse")) %>%
#   mutate(Catégorie = ifelse(Catégorie == "Comité d'éthique", "Comité\nd'éthique", Catégorie)) %>%
#   mutate(Catégorie = ifelse(Catégorie == "Org Internationales", "Organisations\ninternationales", Catégorie)) %>%
#   ggboxplot(x = "Catégorie", y = "nb_citation", fill = "president", xlab = "Type de citation", ylab = "") +
#   labs(fill = "Président") +
#   # ggscatter(x = "date", y = "nb_citation", facet.by = "Catégorie", color = "president",size = 2.5, xlab = "Temps",ylab = "Nombre de citations") +
#   scale_fill_manual(values = c("grey10", "gray50", "gray30", "gray80", "gray70", "gray100")) +
...
# plot1 / plot2
# 
# ggsave("./output/descriptive_citations/BW_citations_catégorie.png", width = 15, height = 12)
```
# BW BIS
```{r}
# Ajouter une colonne président à metadata_avis
...
ggplot(aes(
    ...
    fill = president,
    pattern = president,
    pattern_angle = president,
    pattern_size = president,
    pattern_density = president
  )) +
  ...
  ## Vos palettes
  scale_fill_manual(values = c("grey20", "grey10", "grey10", "grey50", "grey10", "white")) +
  scale_pattern_manual(values = c("none","circle","stripe","none","stripe","none")) +
  scale_pattern_angle_manual(values = c(0, 45, 135, 0, 45, 0)) +
  scale_pattern_size_manual(values = c(0, 0.5, 0.5, 0, 0.5, 0)) +
  scale_pattern_density_manual(values = c(0, 0.7, 0.5, 0, 0.5, 0)) +
  
  labs(fill = "Président", pattern = "Président", 
       pattern_angle = "Président", pattern_size = "Président",
       pattern_density = "Président",
       x = "", y = "") +
  theme_ready(text_size = 28)
```
```{r}
plot2 <- plot_data_nb_citations%>%
  ...
  labs(fill = "Président", pattern = "Président", 
       pattern_angle = "Président", pattern_size = "Président",
       pattern_density = "Président", y = "", x = "Type de citations") +
  theme_ready(text_size = 28) +
  theme(legend.position = "none")
...
```{r}
plot1
ggsave("./output/descriptive_citations/BW_citations_catégorie1.png", width = 15, height = 7)
```
```{r}
plot2
ggsave("./output/descriptive_citations/BW_citations_catégorie2.png", width = 15, height = 6)
```
# BW 
```{r, fig.width=21, fig.height=9.9}
plot_data_nb_citations%>%
  ...
  filter(Catégorie== "Science, littérature")%>%
  ggscatter(x = "date", y = "nb_citation", shape = "president",size = 6, xlab = "Date de publication",ylab = "") +
  theme_ready(text_size = 36)+
  labs(shape = "Président") +
  scale_shape_manual(values = c(1, 17, 0, 16, 2, 15)) +
  geom_text_repel(aes(label = label, fill = "black"), max.overlaps = Inf, size = 8)
```
...
# Graphique
ggscatter(plot_data_nb_citations, x = "date", y = "nb_citation", facet.by = "Désignation",size = 3, color = "president",
  ...
  theme_ready(text_size = 27) +
  geom_text_repel(aes(label = label, color = president), max.overlaps = Inf, size = 8) +
...
# Graphique
ggscatter(plot_data_nb_citations, x = "date", y = "nb_citation", color = "president",size = 4,
