...
Proportion de mots clés dans les phrases écrites sous une président Lecture : Sous Delfraissy, 1.5% des phrases publiées contenaient le mot nature.
...
base_sentences |>
  ...
  labs(title = "Proportion de phrase avec mots clés par president",
       ...
       fill = "Phrase avec") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  stat_pvalue_manual(test_result.t, label = "p.adj.signif", hide.ns = T)+
  stat_anova_test()

if (save_figures)
  ggsave("../output/descriptive_figures/proportion_phrase_vivant_humanité.png")
```
...
cols_to_plt = c("dignité","dignité_humaine")
test_result.t <- base_sentences |>
  pivot_longer(cols = cols_to_plt, names_to = "mot_cle", values_to = "presence") |>
  mutate(mot_cle = factor(mot_cle, levels = cols_to_plt)) |>
  group_by(president, mot_cle, num) |>
  summarise(proportion = 100*mean(presence)) |>
  ungroup()|>
  group_by(mot_cle)|>
...
if (save_figures)
  ggsave("../output/descriptive_figures/proportion_phrase_dignité.png")
```
...
#        fill = "Phrase avec") +
#   theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
#   t_test_pvalue(group.by_var="mot_cle")+
...
#   tag = factor(c("Agent responsable", "Genre/espèce humain.e", "Humain comme empathie", "Individu", "Matériel biologique", "Personne à protéger", "Personne relationnelle", "Poubelle"),
...
#   Freq = c(104, 90,10 , 118, 130, 143, 97, 8)
# ) |>
#   mutate(FreqPerc = 100 * Freq / n_annoté)

stat_annotation <- data.frame(
  tag = factor(c("Corps humain", "Genre/espèce humain.e", "Individu ou sujet", "Personne à protéger", "Personne relationnelle","Personne responsable et autonome", "Poubelle","Sentiment humain"),
               levels = rev(c("Personne responsable et autonome","Personne à protéger","Personne relationnelle","Individu ou sujet","Corps humain","Genre/espèce humain.e","Sentiment humain","Poubelle"))),
  Freq = c(239, 215,235 , 235, 225, 203, 12, 36)
) |>
  mutate(FreqPerc = 100 * Freq / n_annoté)
...
plot_chiffre_temps <- ggplot(base_numbers, aes(x = Date, y = numbers_relative)) +
  ...
  theme_ready()   # Apply the theme_prism from ggprism
if(save_figures)
