...
```{r}
ggplot(metadata_avis, aes(x = factor(num), y = 0)) +
  geom_text(aes(label = paste(num,titre_court, theme, sep = " | ")), angle = 90, vjust = -0.5, check_overlap = TRUE, size = 7) +
  theme_void() +
  theme(
...
participation_categories_across_avis <- grid.arrange(
  ...
  ncol = 1,
  heights = c(1,1)  # Ajuster la hauteur relative pour donner plus d'espace aux textes
)
```
## Alluvial plot
