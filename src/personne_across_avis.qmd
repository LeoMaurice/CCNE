...
```{r}
#| eval: false
# Assuming base_avis is your data frame
title_avis <- base_avis %>%
  arrange(num) %>%
  mutate(date = format(Date, "%d/%m/%Y")) %>%
  select(num, titre_court, theme, date, nb_pages)


# Render the table with scrolling
datatable(title_avis, options = list(scrollX = TRUE, pageLength = 10), rownames = FALSE)  
