...
metadata_auteur_2002 %>%
  mutate(profession_recodée  = as.character(profession_recodée))%>%
  mutate(profession_recodée = case_match(
    profession_recodée,
    "médecin PUPH" ~ "médecin PUPH",
    "chercheur bio" ~ "biologiste",
    "autres professions santé" ~ "professions diverses",
    "politicien ou militant" ~"politicien, militant ou théologien",
    "cadre public/privé" ~ "professions diverses", 
    "journaliste/presse" ~ "professions diverses",
    "théologien" ~ "politicien, militant ou théologien",
    "profession du droit" ~ "profession ou chercheur en droit",
    "chercheur en droit" ~ "profession ou chercheur en droit",
    "chercheur SHS" ~ "chercheur SHS",
    "chercheur sciences exactes" ~ "professions diverses",
    "inconnue" ~ "professions diverses",
    .default = "professions diverses"
  ))%>%
  mutate(profession_recodée = factor(profession_recodée,
                                     levels = c("médecin PUPH", "biologiste", "profession ou chercheur en droit",
                                                "chercheur SHS", "politicien, militant ou théologien", "professions diverses"))) %>%
  mutate(once_rapporteur = ifelse(once_rapporteur,'a été au moins\nune fois rapporteur',"n'a jamais été\n rapporteur"))%>%
  ...
  ungroup() %>%
  select(-total)%>%
  ggbarplot(x='once_rapporteur',
            fill = 'profession_recodée', y = 'percentage',
            position = position_stack(), 
            #label = TRUE,lab.nb.digits = 2,lab.pos = "in",
            ylab = "% des membres", xlab = "sur la période 2002 - 2024")+
  theme_ready()+
...
if(save_figures)
  ggsave("./output/descriptive_members/BW_pourcentage_once_rapporteur_profession.png")
```

```{r}
#| fig-width: 12
#| fig-height: 6
plot_data <- metadata_auteur_2002 %>%
  mutate(profession_recodée  = as.character(profession_recodée)) %>%
  mutate(profession_recodée = case_match(
    profession_recodée,
    "médecin PUPH" ~ "médecin PUPH",
  ...
  mutate(profession_recodée = factor(profession_recodée,
                                     levels = c("médecin PUPH", "biologiste", 
                                                "profession ou chercheur en droit",
                                                "chercheur SHS", 
                                                "politicien, militant ou théologien",
  ...
  ggplot(
    ...
    x = once_rapporteur,
    y = percentage,
    pattern = profession_recodée,
    fill = profession_recodée,
    pattern_angle = profession_recodée,
    pattern_size = profession_recodée
  )) +
  geom_bar_pattern(
    stat = "identity",
    position = position_stack(),
  ...
  scale_pattern_size_manual(values = c(0, 1, 2, 1, 2, 1)) +
  scale_pattern_angle_manual(values = c(0, 45, 75, 135, 15, 45)) +
  scale_fill_manual(values = c("grey20", "grey10", "grey60", "grey10", "grey40", "grey10")) +
  scale_pattern_manual(values = c("none","crosshatch","circle","stripe","circle","stripe")) +
  labs(
    y = "En % des membres", 
    x = "Président du CCNE",
    fill = "Profession",
    pattern = "Profession",
    pattern_angle = "Profession",
    pattern_size = "Profession"
  ) +
  theme_ready() +
  guides(fill = guide_legend(title.position = "top"),
         pattern = guide_legend(title.position = "top")) +
  rotate() +
  grids(axis = "x") +
  theme(
    legend.text = element_text(size = 16),
...
if(save_figures)
  ggsave("./output/descriptive_members/pourcentage_once_rapporteur_profession.png")
...
metadata_avis_membre <- metadata_auteur_2002 %>%
  ...
  left_join(
    metadata_avis%>%
      mutate(
    president = case_when(
      date >= as.Date("2016-12-01") ~ "Delfraissy (2016-)",
      date >= as.Date("2012-01-01") ~ "Ameisen (2012-2015)",
      date >= as.Date("2008-01-01") ~ "Grimfeld (2009-2011)",
      date >= as.Date("1999-01-01") ~ "Sicard (2000-2008)",
      date >= as.Date("1992-01-01") ~ "Changeux (2092-2099)",
      TRUE ~ "Bernard (83-91)"
    )
  )%>%
    select(num, president),
  by = c("avis_rapporteur"="num")
  )%>%
  distinct(id_membre, profession_recodée, president)

# Combiner les trois opérations en une seule
percentage_participation <- metadata_avis_membre %>%
  group_by(president) %>%
  mutate(total_count = n()) %>%
  group_by(president, profession_recodée) %>%
  summarise(
    profession_count = n(),
    total_count = first(total_count)
  ) %>%
  mutate(percentage = 100 * profession_count / total_count) %>%
  select(president, profession_recodée, percentage) %>%
  mutate(
...
ggplot(
  ...
  guides(fill = guide_legend(title.position = "top")) 
  ggsave("./output/descriptive_members/figure6.png")
...
# Graphique en barres empilées
plot_participation_par_avis <- ggplot(percentage_participation, aes(x = factor(president,
                                                                               levels =
                                                                                 c("Sicard (2000-2008)",
                                                                                   "Grimfeld (2009-2011)",
                                                                                   "Ameisen (2012-2015)",
                                                                                   "Delfraissy (2016-)")),
                                                                    y = percentage, fill = profession_recodée)) +
  geom_bar(stat = "identity", position = "stack", color = "black", alpha = 1, size =2) +
  theme_ready(text_size = 37) +
  # theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  # scale_fill_prism(palette = "floral") +
    scale_fill_manual(values = c("grey10", "gray50", "gray30", "gray80", "gray70", "gray90")) +
  labs(fill = "Profession", color = "Profession",
       x = "Président du CCNE", y = "En %") +
  guides(fill = guide_legend(title.position = "top")) 
