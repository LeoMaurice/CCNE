...
BASE_CCNE$Titre <- BASE_CCNE$Titre|>
  str_replace_all("\r", " ") |>
  str_replace_all("\n", " ")
...
BASE_CCNE |> tibble::rowid_to_column("ID") |>
  mutate(full_doc_name = paste(num_avis,Titre,sep = "_"))-> BASE_CCNE
...
load("../data/report/LDA_report_20231123_V3_metrics.RData")
...
```{r}
save(tp_nb,K,diag, file = "../data/report/LDA_report_20231123_V3_metrics.RData")
```
...
full_stm |> select(Annee, starts_with("topic ")) |>
  pivot_longer(-Annee) |> 
  group_by(Annee, name) |> 
  summarize(m = mean(value)) |> 
  mutate(topic_number = as.integer(str_extract(name, "\\d+"))) |>
  ggpubr::ggscatter(x="Annee", y = "m", 
                    facet.by = "topic_number",
                    xlab = "Année",ylab = "Mean Prevalence")+
  geom_line()+
  theme_prism()
```
```{r}
...
```{r}
findThoughts(stm_lda, texts = BASE_CCNE$avis,meta = BASE_CCNE |> select(-avis), topics = 1:num_topic) -> thoughts
