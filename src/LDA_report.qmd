...
Le mot don est avant modification des tokens initiaux le mot le plus fréquent du corpus. Cependant, au regard des 2-grams le suivant présenté dans la table @tbl-contexte_don, on peut voir que cela désigne plusieurs objets différents que nous avons rassemblés en quatre catégories : don de gamètes (d'ovocytes, de spermes etc.), don d'organe, don de sang, don de cellules. Ce découpage a fait perdre sa place au mot don.

Un découpage similaire a été opéré pour le mot 'personne' : on a distingué personne handicapée, personne handicapée mentale, personne malade, personne de confiance, personne agée, personne humaine potentielle (compromis désignant le foetus) en suivant les résultats de la table @tbl-contexte_personne. 
...
A partir des mots les plus fréquents - présentés de façon courte dans la figure @fig-stm-summary_prob et de façon détaillée en annexe dans la figure @tbl-stm-full_prob_words - et des mots les plus exclusifs - de même, résumé figure @fig-stm-summary_frex, détails figure @tbl-stm-full_frex_words, on peut tenter de donner un premier sens à nos topics.

En les prenants dans l'ordre de prévalence :
- Le topic 3 peut correspondre au diagnostic génétique et prénatal.
- Le topic 2 peut correspondre aux questions de procréation et de risques pour l'enfant. L'autisme a - malheuresement - été traité dans ce sens dans un certain nombre d'avis.
- Le topic 9 correspond problablement aux réflexions éthiques portant sur la recherche clinique.
- Le topic 1 correspond aux questions relatives aux prélèvements de tissus humains
- Le topic 6 correspond plus spécifiquement aux questions de recherche sur l'embryon, comme un sous cas du topic 1. Il est important de rappeler que c'est un des thèmes 
- Le topic 10 est difficile à distinguer du topic 9, plus d'explorations sont nécessaires. On peut penser qu'il est lié aux questions de greffes et de transplantations.
- Le topic 5 est un mélange entre les questions de drogue, de prix du médicaments et d'handicaps mentales. Peut-être un lien commun par l'utilisation du vocabulaire de prix ou de travail.
- Le topic 8 semble être un topic sur l'éthique de façon plus générale mais pas forcément sur la recherche clinique, plutôt sur différents problèmes de santé publique : soins, COVID, vaccination, autotests comme le montre les plus exclusifs
- Le topic 4 mélange l'euthanasie et la PMA. C'est possiblement lié à un vocabulaire commun autour de l'assistance, comme sugéré dans les tableaux détaillés
- Le topic 7 correspond aux problématiques d'utilisation des données médicales : RGPD,
...
```
Pour terminer sur la description des topics, on peut regarder pour chaque topic, quels avis montrent la plus forte prévalence de topic. On montre dans la table @tbl-stm-top_avis.
Cela permet de nuancer l'imprétation des topics. Il nou
- Le topic 1 est sur l'utilisation de produits issus du corps humain.
- Le topic 10 est sur l'utilisation de parties du corps humains : greffe mais aussi de façon supprenante un avis sur l'exposition des corps au musée, avis soulevé par une lettre du Museum d'histoire naturel.
- Le topic 11 semble recoupé plusieurs sujets différents. Il peut être interprété comme la monter de l'importance de regard en écosystème les questions de santé : biodiversité et manipulation du génome.
- Le topic 2 semble correspond à sexualité, reproduction et risques avec des avis sur la contraception et l'enjeu du don du sang pour les hommes ayant des rapports avec d'autres hommes.
- Le topic 3 correspond bien au diagnostic prénatal et génétique.
- Le topic 4 semble correspondre à la fin de vie, plus la santé en prison. Cela est dû à un grand volet sur la fin de vie en prison dans l'avis n°94.
- Le topic 5 correspond à l'usage des drogues et les risques de certains traitements médicamenteux (centoxin, anti-corps autorisé en 1991, puis retiré en 1993 en raison de falsifications). On peut l'interpréter comme substances et risques.
- Le topic 6 est bien le topic embryon.
- Le topic 7 est le topic données de santé.
- Le topic 8 correspond à l'éthique en situation de santé publique : pandémies, vaccinations, santé des migrants.
- Le topic 9 correspond à éthique et recherche, notamment l'étonnant avis n°131 dédié aux expérimentations en sciences de l'éducation, sciences cognitives et psychologie, ce qui explique la prévalence du terme pédagogie.
...
On obtient finalement le tableau d'interprétation grossière suivant :
| Numéro du Topic | Interprétation Grossière               |
|-----------------|---------------------------------------|
...
- pour le topic 1, utilisation du corps humains : croit avec la thématique greffe, ce qui est logique vu qu'il semble parler d'utiliser les tissus humains. Il est intéressant de noter que c'est un sujet qui décroit avec tous les présidents depuis 1992.
- pour le topic 2, effets positifs et significatifs (respectivement à 1% et à 10%) des thèmes sexualité, genre et du thème procréation. Ce qui est logique pour le topic associé aux risques de ces pratiques (?).
- pour le topic 3, c'est un topic extrêment transversal aux questionnements du CCNE.
- pour le topic 4, on a un effet très très fortement significatif du thème fin de vie ce qui confirme notre interprétation
- pour les topic 5, 6 et 11, les effets et nos interprétations sont complexes à mettre en lien
- pour les topics 7 et 8, on a bien l'influence des questions de santé publique, ce qui est logique avec nos interprétations.
- pour le topic 9, on trouve la significativité des politiques de santé et de la recherche.
- pour le topic 10, on retrouve l'influence du corps (thème des greffes) mais surtout l'importance du droit.
...
```{r}
#| eval: true
stm_lda.effect <- estimateEffect(formula = 1:11 ~ saisine + divergence + factor(president) + factor(theme), stmobj = stm_lda, metadata = stm_data$meta, uncertainty="Global")
```
...
for(i in 1:11){
  topic_caption <- paste("Effets structurels significatifs sur la prévalence des thèmes pour le topic", i,
                         topic_names[paste0("Topic",i)])
  print(summary_stm_main_effects$tables[[i]] |>
    as.data.frame()|>
    mutate(signif = sapply(`Pr(>|t|)`,signif_codes))|>
    filter(`Pr(>|t|)`<=0.1)|>
      kable(caption = topic_caption)|>
...
#| eval: true
#| output: true
#| fig-cap: "Ensemble des mots les plus fréquents par topic"
#| label: tbl-stm-full_prob_words
#| 
plot(stm_lda, type = "labels", labeltype = "prob")
```

```{r}
#| eval: true
#| output: true
#| fig-cap: "Ensemble des mots les plus exclusifs par topic"
...
#| eval: true
#| output: true
#| fig-cap: "Full regression like table of the structural effects on each topics"
#| label: tbl-stm-full_reg_tab
summary(estimateEffect(formula = 1:11 ~ saisine + divergence + factor(president) + factor(theme) + s(Annee), stmobj = stm_lda, metadata = stm_data$meta, uncertainty="Global"))
