...
metadata_avis %>%
  filter(sapply(ids_rapporteurs, function(x) any(is.na(x)))) %>%
  mutate(string_ids_rapporteurs = sapply(ids_rapporteurs, function(x) paste(x, collapse = ", ")))%>%
  select(num, rapporteurs, string_ids_rapporteurs)%>%
  View()
```
...
#| eval: false
metadata_avis %>%
  filter(sapply(ids_membres_gt, function(x) any(is.na(x)))) %>%
  mutate(string_ids_membres_gt = sapply(ids_membres_gt, function(x) paste(x, collapse = ", ")))%>%
  select(num, membres_gt, string_ids_membres_gt)%>%
  View()
...
On se limite aux membres du CCNE qui l'ont rejoint après (ou égale) à 2002, car c'est à partir de là qu'on a des informations quasi exshaustives sur les auteurs des avis.
...
#| fig-height: 10
metadata_auteur_2002|>
  gghistogram(x = "nb_avis_as_membre_gt", fill = "sexe",#fill = "#004080",
              position = position_stack(),
              ylab = "Nombre de membre", xlab = "Nombre de fois membre d'un gt")+
  theme_ready()+
  scale_fill_prism(palette = "floral")

if(save_figures)
  ggsave("../output/descriptive_members/histo_nb_membre_gt.png")
```
```{r}
...
#| fig-height: 10
metadata_auteur_2002|>  
  ggviolin(x='président_première_nomination',fill = 'once_rapporteur', y = 'durée_mandat',
            ylab = "Durée total du mandat", xlab = "Président à la première nomination",
            title = "Durée total des mandats effectués au CCNE\nselon le président à leur première nomination\net si a été au moins une fois rapporteur")+
  theme_ready()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  scale_fill_prism(palette = "floral")+
  labs(fill = "A été au moins une fois rapporteur")
if(save_figures)
  ggsave("../output/descriptive_members/durée_mandat_once_rapporteur_président.png")
```

Sans trop de surprise, les membres avec des mandats qui ont durés longtemps, ont souvent plus au moins une fois rapporteurs et inversement.

```{r}
#| fig-width: 12
#| fig-height: 10
metadata_auteur_2002|>  
  ggviolin(x='président_première_nomination',fill = 'once_rapporteur', y = 'age_première_nomination',
            ylab = "Age à la première nomination", xlab = "Président à la première nomination",
            title = "Age à la première nomination au CCNE\nselon le président à leur première nomination\net si a été au moins une fois rapporteur")+
  theme_ready()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  scale_fill_prism(palette = "floral")+
  labs(fill = "A été au moins une fois rapporteur")
if(save_figures)
  ggsave("../output/descriptive_members/age_première_nomination_once_rapporteur_président.png")
```
Peut être une légère tendance à ce que les plus jeunes s'investissent plus mais rien d'évident.

```{r}
#| fig-width: 12
#| fig-height: 10
metadata_auteur|>  
  ggboxplot(x='profession_recodée',fill = 'profession_recodée', y = 'nb_avis_as_rapporteur',
            #facet.by = 'président_première_nomination',
            ylab = "Nombre d'avis", xlab = "Profession",
            title = "Nombre d'avis en tant que rapporteur\nselon leur profession")+
  theme_ready()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  scale_fill_prism(palette = "floral")+
  labs(fill = "Profession")+
...
if(save_figures)
  ggsave("../output/descriptive_members/nb_avis_as_rapporteur_profession.png", width = 10, height = 8)
```
...
metadata_auteur_2002 %>%
  ...
  ungroup() %>%
  select(-total)%>%
  ggbarplot(x='profession_recodée',fill = 'once_rapporteur', y = 'percentage',
            position = position_stack(), 
            #label = TRUE,lab.nb.digits = 2,lab.pos = "in",
            ylab = "% des membres", xlab = "Profession",
            title = "Pourcentage des membre du CCNE\nselon leur profession\net selon le fait d'avoir participé en tant que rapporteur")+
  theme_ready()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  scale_fill_prism(palette = "floral")+
  labs(fill = "A participé en tant que rapporteur")+
  guides(fill = guide_legend(title.position = "top"))
if(save_figures)
  ggsave("../output/descriptive_members/pourcentage_profession_once_rapporteur.png", width = 8, height = 10)
```
...
metadata_auteur_2002 %>%
  ...
  group_by(nb_avis_as_rapporteur_discretized, président_première_nomination) %>%
  summarise(count = n(), .groups = 'drop') %>%
  group_by(président_première_nomination) %>%
  mutate(total = sum(count), percentage = 100 * count / total) %>%
  ungroup() %>%
  select(-total)%>%
  ggbarplot(x='président_première_nomination',fill = 'nb_avis_as_rapporteur_discretized', y = 'percentage',
            position = position_stack(), 
            #label = TRUE,lab.nb.digits = 2,lab.pos = "in",
            ylab = "% des membres", xlab = "Président à la première nomination",
            title = "Pourcentage des membre du CCNE\nselon le président à leur première nomination\net selon le nombre d'avis en tant que rapporteur")+
...
if(save_figures)
  ggsave("../output/descriptive_members/pourcentage_président_nb_avis_as_rapporteur.png", width = 8, height = 10)
```
```{r}
metadata_auteur_2002|>
  ggscatter(x = "durée_mandat",y = "nb_avis_as_rapporteur", color = "sexe",
            xlab = "Durée totale du mandat", ylab = "Nombre d'avis en tant que rapporteur")+
  theme_ready()+
  scale_fill_prism(palette = "floral")
```
```{r}
metadata_auteur_2002|>
  ggscatter(x = "durée_mandat",y = "nb_avis_as_participant", color = "sexe",
            xlab = "Durée totale du mandat", ylab = "Nombre d'avis en tant que participant")+
  theme_ready()+
  scale_fill_prism(palette = "floral")
```
...
#| fig-height: 10
metadata_auteur_2002 %>%
  group_by(once_rapporteur, président_première_nomination, profession_recodée) %>%
  summarise(count = n(), .groups = 'drop') %>%
  group_by(once_rapporteur,président_première_nomination) %>%
  mutate(total = sum(count), percentage = 100 * count / total) %>%
  ungroup() %>%
  select(-total)%>%
  ggbarplot(x='président_première_nomination',fill = 'profession_recodée', y = 'percentage',
            facet.by = 'once_rapporteur',
            position = position_stack(), 
            #label = TRUE,lab.nb.digits = 2,lab.pos = "in",
            ylab = "% des membres", xlab = "Président à la première nomination",
            title = "Pourcentage des membre du CCNE\nselon le président à leur première nomination\net selon le nombre d'avis en tant que rapporteur")+
...
if(save_figures)
  ggsave("../output/descriptive_members/pourcentage_président_once_rapporteur_profession.png", width = 8, height = 10)
```
```{r}
#| fig-width: 12
#| fig-height: 10
...
```{r}
membres_ccne_qgs <- inner_join(metadata_auteur_2002,QGS%>%
                                 select(id_membre,nb_affiliations), by = "id_membre")
```

```{r}
membres_ccne_qgs|>
  ggscatter(x = "nb_avis_as_participant",y = "nb_affiliations", shape = "sexe", color = "profession_recodée",
...
membres_ccne_qgs|>
  ggscatter(x = "nb_avis_as_rapporteur",y = "nb_affiliations", shape = "sexe", color = "profession_recodée",
            size = 4,
...
if(save_figures)
  ggsave("../output/descriptive_members/rapporteur_vs_qgs.png", width = 8, height = 10)
```