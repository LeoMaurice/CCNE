...
df_heatmap <- base_sentences |> 
  ...
  summarise(humain = sum(humain)/n_in_quantile,
            personne = sum(personne)/n_in_quantile,
            individu = sum(individu)/n_in_quantile,
            nature = sum(nature)/n_in_quantile,
            vivant = sum(vivant)/n_in_quantile,
            environnement = sum(environnement)/n_in_quantile)
```
Description :
...
dev.off()
```
# Heatmap science and person words
```{r}
# Assuming you have a data frame called 'base_sentences' with columns: num, human, person, societe, nature, position_in_avis
quantile = 50
...
cor_matrix <- cor(base_sentences|>ungroup()|>
...
df_heatmap <- base_sentences |>
  ...
  mutate(numbers_count = count_digits(sentence),
         mots_count = count_words(sentence)) |>
  ungroup() |>
  mutate(position_in_avis = sentence_index/number_sentences)|>
  arrange(num, position_in_avis) |>
  group_by(num) |>
  ...
  ungroup()|>
  group_by(num,quantile_group,president,theme,number_sentences)|>
  summarise(numbers_count_grouped = sum(numbers_count),
            mot_count_grouped = sum(mots_count),
            numbers_count_by_sentence = sum(numbers_count)/n(),
            n_in_quantile = n(),
            na.rm = TRUE)|>
  mutate(numbers_count_relative = log(1+numbers_count_grouped/mot_count_grouped))

source("helpers/heatmap.R")
plot_width = 13
plot_height = 8

pdf("../output/descriptive_figures/raw_number_words_complexheatmaps.pdf", width = plot_width, height = plot_height)  # Adjust size as needed

heatmap_figure_frequence_quantile(df_heatmap = df_heatmap,
                                  X="num",Y="quantile_group",
                                  Zs = c("numbers_count_relative"),
                                  names_legend = c("Nombre"),
                                  subrow_title = "Nombre d'occurence/Nombre de mots par phrase",
                                  subcol_title = "Numéro de l'avis",
                                  mainrow_title = "Position relative de la phrase",
                                  maincol_title = "Cartes de chaleur de occurence de nombre par groupe de phrase (2%)",
                                  colors_hexcodes = c("#66CCFE")
                                  )
dev.off()
```
...
plot_chiffre_temps <- ggplot(df_timeline, aes(x = Date, y = numbers_relative)) +
